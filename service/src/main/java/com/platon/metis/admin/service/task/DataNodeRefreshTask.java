...
public class DataNodeRefreshTask {
    ...
    public void task() {
        ...
        try {
            List<RegisteredNodeResp> registeredNodeRespList = yarnClient.getDataNodeList();
            List<DataNode> dataNodeList = new ArrayList<>();
            if (!CollectionUtils.isEmpty(registeredNodeRespList)) {
                registeredNodeRespList.forEach(item -> {
            ...
            log.error("定时刷新数据节点的和调度服务的连接状态出错", e);
        }
