...
CREATE TABLE `local_seed_node` (
  ...
  `seed_node_id` varchar(256) DEFAULT NULL COMMENT '发布后底层返回的host唯一ID',
...
CREATE TABLE `local_data_node` (
  ...
  `host_Name` varchar(32) DEFAULT NULL COMMENT '节点名称',
  `internal_IP` varchar(32) DEFAULT NULL COMMENT '节点内部IP',
  `internal_Port` INT DEFAULT NULL COMMENT '节点内部端口',
...
CREATE TABLE `local_power_history` (
  ...
  `refresh_status` varchar(2) DEFAULT NULL COMMENT '刷新时间标志（0：小时，1：天）',
...
CREATE TABLE `task` (
    ...
    `task_Name` varchar(32) DEFAULT NULL COMMENT '任务名称',
    `owner_Identity_id` varchar(256) DEFAULT NULL COMMENT '任务发起方身份',
    `apply_use` varchar(256) DEFAULT NULL COMMENT '发起任务的用户的信息 (task是属于用户的)',
    `user_type` int(4) DEFAULT '0' COMMENT '发起任务用户类型 (0: 未定义; 1: 以太坊地址; 2: Alaya地址; 3: PlatON地址)',
