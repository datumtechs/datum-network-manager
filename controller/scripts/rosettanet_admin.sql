...
(
    ...
    conn_status    VARCHAR(5) COMMENT '节点状态，-1: 未被调度服务连接上; 0: 连接上; 1: 算力启用<计算服务>; 2: 算力被占用(计算服务算力正在被任务占用)',
    conn_message   VARCHAR(32) COMMENT '节点(连接失败)信息',
    power_id       VARCHAR(128) COMMENT '节点启动后底层返回的算力ID',
    power_status    VARCHAR(20) COMMENT '算力状态 (create: 还未发布的算力; release: 已发布的算力; revoke: 已撤销的算力)',
...
(
    ...
    owner_identity_id VARCHAR(128) COMMENT '发起方ID',
    owner_identity_name VARCHAR(128) COMMENT '发起方名称',
    task_start_time   DATETIME     COMMENT '发起时间',
    result_side_id       VARCHAR(128)  COMMENT '结果方ID',
    result_side_name       VARCHAR(32)  COMMENT '结果方名称',
	coordinate_side_id   VARCHAR(128)  COMMENT '协作方ID',
	coordinate_side_name   VARCHAR(32)  COMMENT '协作方名称',
...
CREATE TABLE `task` (
  `id` int(11) NOT NULL AUTO_INCREMENT COMMENT '序号',
  `task_Id` varchar(128) COLLATE utf8mb4_unicode_ci NOT NULL COMMENT '任务ID',
...
) ENGINE=InnoDB AUTO_INCREMENT=85 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='全网任务表 用于同步本地任务数据以及全网的相关数据';;
