...
CREATE TABLE `local_meta_data` (
  ...
  `status` INT NOT NULL DEFAULT 0 COMMENT '元数据的状态 (0: 未知; 1: 还未发布的新表; 2: 已发布的表; 3: 已撤销的表; 4：删除的;5: 发布中; 6：撤回中)',
...
CREATE TABLE `local_power_node` (
    ...
    `power_status` tinyint(4) DEFAULT 0 COMMENT '算力状态 (0: 未知; 1: 还未发布的算力; 2: 已发布的算力(算力未被占用); 3: 已发布的算力(算力正在被占用); 4: 已撤销的算力;5: 发布中; 6：撤回中)',
